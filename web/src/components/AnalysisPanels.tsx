...
const ChipRow = ({ label, net }: { label: string; net: number | undefined }) => {
  if (net === undefined) {
    return (
...
const Fundamentals = ({ kpi }: { kpi: Kpi }) => (
...
const ChipsView = ({ chips }: { chips: Chips }) => (
  <>
    ...
    {chips && <div className="mt-3 text-xs text-slate-500">資料日期：{chips.date}</div>}
  </>
);

const Technicals = ({ kpi, quote }: { kpi: Kpi; quote: Quote }) => (
